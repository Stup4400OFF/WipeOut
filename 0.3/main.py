    ...
    while True:
        ...
        if query in ["reset", "clear", "cls"]:
            ...
            try:
                ...
                print("Cannot remove file (If I directory use remdir or remove)")
            except FileNotFoundError:
                print("File not found (Use tree to see all your files)")
        elif query in ["dir", "new"]:
            directory = input("Directory name: ")
            os.mkdir(f"DATA\\sys\\{directory}")
        elif query in ["tree", "get"]:
            print(VAR.get_tree())
        elif query in ["remdir", "remove"]:
            print(">WARNING<\nThis will remove the input file name")
            directory = input("Directory name: ")
            try:
                shutil.rmtree(f"DATA\\sys\\{directory}")
            except OSError as e:
                print(e)
